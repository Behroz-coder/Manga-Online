...
const configPath = path.join(os.homedir(), ".manga-online", "secret-config.json");
const oldPath =
...
if (!fs.existsSync(configPath)) {
	...
	if (fs.existsSync(oldPath)) {
		console.error(
			chalk.red("[SECRET]") +
				` Found secret-config.json in the \`src\` folder. This location is deprecated and is moved to \`~/.manga-online/secret-config.json\`.`
		);
